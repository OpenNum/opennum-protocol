...
const { fetchOrdinalsOwner } = require('./_ordinals');
...
async function verifyAction({
  ...
  if (requireOwnership) {
    ...
    const ownership = await fetchOrdinalsOwner(inscriptionId, {
      userAgent: 'OpenNum-Auth/1.0 (opennum.org)'
    });
    if (!ownership.verified || !ownership.owner) {
      return authError(503, 'On-chain ownership could not be verified. Please try again.');
    }
