...
function sanitizeSocialUrl(val, key) {
  if (!val) return null;
  const raw = String(val).trim();
  if (!raw) return null;
  const withoutAt = raw.replace(/^@+/, '');
  if (!/^https?:\/\//i.test(raw)) {
    if (key === 'x') return sanitizeUrl(`https://x.com/${withoutAt}`);
    if (key === 'instagram') return sanitizeUrl(`https://instagram.com/${withoutAt}`);
    if (key === 'telegram') return sanitizeUrl(`https://t.me/${withoutAt}`);
    if (key === 'website') return sanitizeUrl(`https://${raw}`);
  }
...
function sanitizeLinks(links) {
  ...
  ['x', 'instagram', 'telegram', 'website'].forEach((key) => {
    const input = key === 'x' ? (links.x || links.twitter) : links[key];
    const val = sanitizeSocialUrl(input, key);
    if (val) clean[key] = val;
  });
  return clean;
}

function parseInscriptionNumber(value) {
