...
- [x] Cursed Inscription 规则已定义 — `#c-1234` 格式已写入协议，托管网站/API 实现仍待完成
- [x] `.btc` 显示别名规则已定义 — 托管 indexer 的自动识别仍待完成
- [x] 白皮书发布（英文 / 中文）
- [x] GitHub 仓库开放：`github.com/OpenNum/opennum-protocol`

### 网站与身份页面
- [x] `opennum.org` 在 Vercel 上线
- [x] 动态身份页面：`opennum.org/n/:number`，由 Profile API 驱动
- [x] 身份页面包含：状态标签、关联钱包、铭文元数据和 resolver-backed profile 数据
- [x] `opennum.org/explore` — Explorer 页面，含实时统计、身份列表、卡片网格、搜索功能、Resolver API 预览
...
### Resolver API
- [x] `GET /api/resolve?num=:number` → 返回钱包地址 / 状态
- [x] `GET /api/profile?num=:number` → 完整个人资料元数据，并尽力补充 Ordinals 元数据
- [x] `GET /api/list` → Explorer 使用的实时 registry 列表
- [x] 兼容别名：`/api/v1/resolve/:number`、`/api/v1/profile/:number`、`/api/v1/register`、`/api/v1/list`
- [x] 参考 Indexer 部署上线，在可访问链上数据时验证注册记录
- [x] Explorer 接入真实 API 数据

### 注册页面
- [x] `opennum.org/register` — 用户通过比特币钱包对规范消息签名完成注册
- [x] Unisat 钱包连接流程
- [x] 页面内生成签名并提交 `POST /api/register`
...
- [ ] `GET /api/profile?num=:number` 与 `/api/v1/profile/:number` 返回完整的结构化 Profile 数据
...
| 阶段 | 时间节点 | 核心交付物 | 状态 |
|------|----------|-----------|------|
| **第一阶段 — 基础建设** | 2026 Q1 | 协议规范 · 双锚点 · 状态机 · AI Agent v1.1 · Cursed Inscriptions（协议）· 网站 · 身份页面 · Explorer | ✅ 已基本完成 |
...
| 比特币原生 | ✅ | ❌ | ✅ |
| 防版本争议 | ✅ 双锚点设计 | 不适用 | ❌ |
| AI Agent 委托 | 协议已定义；应用待实现 | ❌ | ❌ |
| Cursed Inscription 支持 | 协议已定义；应用待实现 | 不适用 | 不一 |
